"""
agents/permits_agent.py  v3
━━━━━━━━━━━━━━━━━━━━━━━━━━━
🏗️ Permisos de Construcción — Bay Area Completa

⚡ OPTIMIZACIONES v3:
  - Fetch de ciudades EN PARALELO (ThreadPoolExecutor) — de ~17s → ~3s
  - Contactos via índice de tokens (O(k) en lugar de O(52k))
  - Agente singleton (instanciado 1x desde main.py)
  - Cache de CSLB por GC para no repetir peticiones HTTP

FIXES heredados de v2:
  ✅ SF  — KeyError 'permit_number' corregido
  ✅ SJ  — migrado a CKAN JSON (Socrata 404)
  ✅ OAK — marcado skip (Accela, sin API pública)
  ✅ BRK — marcado skip (requiere Socrata token)
"""
...
# Número de ciudades que se consultan en paralelo
...
PERMIT_SOURCES = [
    ...
    {
        "city": "San Jose", "engine": "ckan_json",
    ...
    },
    {
        ...
        "params": {"$limit":50,"$order":"issued_date DESC","$where":"permit_status='Issued'"},
        "field_map": {
    ...
    },
    {
        ...
        "params": {"$limit":50,"$order":"issue_date DESC","$where":"status='Issued'"},
        "field_map": {
            "id":"permit_number","address":"address","address2":None,
    ...
    },
    {
        ...
        "params": {"$limit":50,"$order":"date_issued DESC","$where":"status='ISSUED'"},
        "field_map": {
    ...
    # Oakland — Accela (sin API pública), skip silencioso
    {
    ...
    # Berkeley — requiere Socrata App Token (SOCRATA_APP_TOKEN en .env)
    {
        "city": "Berkeley", "engine": "socrata", "_skip_if_no_data": True, "_requires_token": True,
        "url": "https://data.cityofberkeley.info/resource/cqze-unm8.json",
        "params": {"$limit":50,"$order":"date_issued DESC","$where":"permit_status IN('ISSUED','FINALED')"},
        "field_map": {
...
def _fetch_ckan_json(source: dict) -> list:
...
def _fetch_source(source: dict) -> tuple:
    """Descarga una fuente — llamado en paralelo. Retorna (city, records, error)."""
    city = source["city"]
    try:
        if source.get("engine") == "ckan_json":
...
class PermitsAgent(BaseAgent):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._contacts = load_all_contacts()   # 0ms (cache de módulo)
        self._cslb_cache: dict = {}
    ...
    def fetch_leads(self) -> list:
        """⚡ Consulta todas las ciudades EN PARALELO."""
        all_leads = []
        active_sources = [
            s for s in PERMIT_SOURCES
        ...
        with ThreadPoolExecutor(max_workers=PARALLEL_CITIES) as executor:
            futures = {executor.submit(_fetch_source, s): s for s in active_sources}
            ...
            for fut in as_completed(futures):
                ...
                city_str, records, error = fut.result()
    ...
    def notify(self, lead: dict):
        ...
        fields = {
            ...
            "💰 Valor Estimado":   f"${float(lead['value']):,.0f}" if lead.get("value") else "—",
        }
